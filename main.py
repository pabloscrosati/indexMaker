...
# Import dependencies
import glob, os, textwrap, numpy, time, argparse, sys
...
def residue_guan_pairs(coord_x, coord_y, coord_z, prot_index, guan_index, resid, resname, atomname, success_guan,
                       cutoff=short_cutoff):
    ...
    if number_of_chains > 1:
        for chain in range(number_of_chains):
            ...
            pair_list, gro_index = residue_guan_pairs(x_c, y_c, z_c,
                                                      ...
                                                      guan_indices, resid, resname, atomname, success_guan)
            write_file(pair_list, "protein_chain_%s.txt" % chain)
    else:
